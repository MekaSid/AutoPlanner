...
def create_individual(hotels, activities):
    individual = {
...
def create_population(size, hotels, activities):
    return [create_individual(hotels, activities) for _ in range(size)]
...
def selection(population, population_size, trip_length, budget):
    fitness_with_args = partial(fitness, budget=budget, trip_length=trip_length)
    population.sort(key=fitness_with_args, reverse=True)
    filtered_population = [individual for individual in population if fitness(individual, budget, trip_length) > 0]
...
def mutate(individual, hotels, activities, mutation_rate):
...
def genetic_algorithm(hotels, activities, trip_length, budget, population_size, generations, mutation_rate):
    population = create_population(population_size, hotels, activities)
    for _ in range(generations):
        selected_population = selection(population, population_size, trip_length, budget)
        new_population = []
        while len(new_population) < population_size:
            parent1, parent2 = random.sample(selected_population, 2)
            child1, child2 = crossover(parent1, parent2)
            mutate(child1, hotels, activities, mutation_rate)
            mutate(child2, hotels, activities, mutation_rate)
            new_population.extend([child1, child2])
        population = new_population
    best_individual = max(population, key=partial(fitness, budget=budget, trip_length=trip_length))
...
def run_multiple_times(runs, budget, hotels, activities, trip_length, population_size, generations, mutation_rate):
    best_individual = None
    best_fitness = 0
    
    for _ in range(runs):
        individual = genetic_algorithm(hotels, activities, trip_length, budget, population_size, generations, mutation_rate)
        individual_fitness = fitness(individual, budget, trip_length)
        
        if individual_fitness > best_fitness and budget >= calculate_cost(individual, trip_length):
...
"""def main(): 
    ...
    best_individual = run_multiple_times(250, budget, hotels, activities, trip_length, population_size, generations, mutation_rate)
    ...
    if best_individual:
        ...
        total_cost = calculate_cost(best_individual, trip_length)
